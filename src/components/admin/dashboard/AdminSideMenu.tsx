...
const AdminSideMenu: React.FC = () => {
  ...
  const handleMenuItemClick = (id: string) => {
    ...
    // Toggle expanded state if the item has subitems
    const item = [...menuItems, ...templateItems].find(item => item.id === id);
    if (item?.subItems) {
      setExpanded(expanded === id ? null : id);
    }
  };

  const renderMenuItems = (items: MenuItem[], isTemplate = false) => {
    return (
  ...
  return (
    <div className="w-64 h-screen bg-blue-900 text-white border-r border-blue-800 flex flex-col">
      <div className="p-4 flex items-center border-b border-blue-800">
        <span className="font-bold text-xl">Light Blue</span>
      </div>
      
      <div className="flex-1 overflow-y-auto p-4">
        <div className="mb-4">
          <div className="text-xs font-semibold text-blue-300 mb-2 uppercase tracking-wider">
            YOU ARE HERE: <span className="text-gray-400">Visits</span>
          </div>
        ...
        <div className="mb-2">
          <div className="text-xs font-semibold text-blue-300 uppercase tracking-wide">TEMPLATE</div>
        </div>
        
        <nav>
          {renderMenuItems(templateItems, true)}
        </nav>
      </div>
    </div>
  );
